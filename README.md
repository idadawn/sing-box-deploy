...
Debian / Ubuntu 一键部署 **Trojan + Hysteria2** 中继服务器。当前仅部署到 `T` 服务器；AI 与普通流量由 `ISP-1` 和可选 `ISP-2` SOCKS5 出口承担，也可在 T 服务器启用视频/CDN/软件包下载直出。J 服务器已退役，脚本会拒绝任何 `J_*` 配置，订阅只保留 T 节点与 ISP 出口容灾，以及每日更新的 [Loyalsoldier/clash-rules](https://github.com/Loyalsoldier/clash-rules) 规则集。
...
- `Clash` 订阅内置自动容灾组，仅在 `T-ISP1/T-ISP2` 节点之间自动切换
- `v2rayN / v2rayNG` 订阅直接下发 `T-ISP1-*`、`T-ISP2-*` 节点，按需手动选择
