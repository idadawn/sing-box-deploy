...
- `/c` 与 `/tx` 的默认 DNS 经代理查询，仅明确的国内和私有域名使用国内直连解析，避免未知海外域名经并发备用解析泄露。节点域名保留独立引导解析；全局扩展脚本与客户端自定义 DNS 覆盖需单独配置。
...
- 自动续费且不设到期日的条目，将最后一列设为 `never`。订阅响应使用 `expire=0` 表示无限期，首页显示“有效期不限”；混合订阅仍以其中最早的有限到期日作为提示。此设置不代替供应商的续费与账号状态管理。
...
当前 tx 清单包含 `192.129.0.0/16` 和 `192.132.0.0/16`，两个网段均以高优先级 `DIRECT,no-resolve` 规则下发；更新订阅后继续保留。Clash 的系统代理绕过表属于客户端本机设置，不由订阅修改。
