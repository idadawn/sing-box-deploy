...
回退，不参与正常流量分担。客户端继续使用原有订阅 URL，刷新订阅后获取硅谷
专用入口域名，避免逐台手工修改节点。

- Trojan：`tj.1761.org`
- Hysteria2：`hy.1761.org`

东京原入口域名保持不变，迁移期可随时回退。
...
把 Pages 订阅发布为 `tj.1761.org` 与 `hy.1761.org` 前必须同时满足：
...
两个硅谷域名已经独立解析到 `43.162.81.53`。发布新订阅后东京服务器至少保留
一个 DNS TTL 周期；客户端验证无误后，再把规则同步定时器和本地 `tx` SSH
别名迁移到硅谷机。

## 当前迁移阻塞

`ds-1`、`ds-2`、`ds-3` 已能从硅谷机访问。`dawn` 的 SOCKS5 当前拒绝硅谷来源，
需要在上游放行 `43.162.81.53`。

东京机访问硅谷入口时，TCP/UDP 流量未到达硅谷网卡，说明云安全组或外层网络
ACL 尚未对公网开放八个业务端口。以上两个问题解决前不得发布硅谷订阅。
