...
硅谷服务器 `43.162.81.53` 是新的唯一接入数据面。东京服务器在迁移期只作为
回退，不参与正常流量分担。客户端继续使用原有订阅和入口域名，避免逐台修改。
...
切换 `trojan.1761.org` 与 `hy2.1761.org` 前必须同时满足：
...
DNS TTL 为 300 秒。切换后东京服务器至少保留一个 TTL 周期；客户端验证无误后，
再把规则同步定时器和本地 `tx` SSH 别名迁移到硅谷机。

## 当前迁移阻塞

`ds-1`、`ds-2`、`ds-3` 已能从硅谷机访问。`dawn` 的 SOCKS5 当前拒绝硅谷来源，
需要在上游放行 `43.162.81.53` 后才能通过迁移门禁。在此之前不得切 DNS。
